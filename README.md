...
## 🖥️ 系统运行界面 (DApp Console Screens)

本项目提供了一个精美的现代化 Web 控制台，用于演示完整的 DKG 设置、隐私钱包和合规审计审查流程：

### 1. DKG 分布式密钥生成控制台
展示了 5 个审计节点进行本地多项式生成、通信密钥注册、加密碎片分发以及最终聚合公示全局审计公钥的完整流程：
![DKG 注册与监控控制台](docs/images/dkg_console.png)

### 2. 用户隐私钱包页面
提供隐私代币 $MCK$ 的存入（Deposit）、隐私转账（Transfer，包含 E2E 接收方加密与门限合规加密）以及提款（Withdraw）操作：
![用户隐私钱包](docs/images/user_wallet.png)

### 3. 合规审查控制台
展示了合规审计人员如何切换钱包以收集 3/5 个节点的局部解密份额，最终门限解密还原出隐私转账明细（包括实际发送方、接收方、转账金额与合规结论）：
![合规审查与门限解密](docs/images/compliance_audit.png)
